...
{
    if(!SQL_CheckConfig("multi"))
    {
        SetFailState("Not found section \"multi\" in databases.cfg");
        return;
    }

    char buffer[PLATFORM_MAX_PATH];
    BuildPath(Path_SM, buffer, sizeof(buffer), "configs/multi.ini");
    kv = CreateKeyValues("Settings");
    kv.ImportFromFile(buffer);
    Database.Connect(SQLCallBack, "multi");
...
public void SQLCallBack(Database db, const char[] error, any data)
{
    ...
    char sQuery[256];
    Format(sQuery, sizeof(sQuery), "CREATE TABLE IF NOT EXISTS `multi` (`active` INTEGER(10) NOT NULL, `mode` VARCHAR(64) NOT NULL PRIMARY KEY)");
    ...
    do{
        ...
        Format(sQuery, sizeof(sQuery), "INSERT INTO `multi` (`active`, `mode`) VALUES ('0', '%s')", ModeInDb);
...
public Action LoadConfig(Handle hTimer)
{
    ...
    Format(sQuery, sizeof(sQuery), "SELECT * FROM `multi` WHERE `active`='1'");

    DBResultSet result = SQL_Query(gDatabase, sQuery);
    
    if(result == INVALID_HANDLE || !result.HasResults)
    {
        SetFailState("Сan`t select from database OnConfigExec");
        delete result;
        return Plugin_Stop;
    }
    ...
    if(result.RowCount != 0)
    {
        ...
        Format(sQuery, sizeof(sQuery), "UPDATE `multi` SET `active`='0' WHERE `mode`='%s'", ActiveMode);
        SQL_Query(gDatabase, sQuery);
    }
    delete result
...
public int MapsMenu(Menu menu, MenuAction action, int client, int item)
{
    switch(action)
    {
        ...
        case MenuAction_Select:
        {
			...
			Format(sQuery, sizeof(sQuery), "UPDATE `multi` SET `active`='1' WHERE `mode`='%s'", map);
			strcopy(globalmod, sizeof(globalmod), map);
			DataPack dp = CreateDataPack();
			kv.Rewind();
			CreateTimer((kv.GetFloat("countdown_time")+1.0), Execution, dp);
			gcounter = RoundToFloor(kv.GetFloat("countdown_time"));
		...
		// {
			...
			// {
				...
				// {
					...
					// IntToString(i, temp, sizeof(temp));
					// kv.GetString(temp, section, sizeof(section));
					// if(section[0] != '\0') ServerCommand("sm plugins load %s", section);
					// else break;
				// }
			// }
		// } while(kv.GotoNextKey())
